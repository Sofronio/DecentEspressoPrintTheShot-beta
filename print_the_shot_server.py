...
def _version_key(v):
    ...
    第四位是「是否正式版」的哨兵:0 = 预发布,1 = 正式版。这样 '2.0' > '2.0-beta.9'。
    ...
    were already current when they were not. The more often a prerelease ships, the
    worse this gets.

    The prerelease number is now part of the comparison. The fourth element is a
    sentinel: 0 for a prerelease, 1 for a final release, so '2.0' > '2.0-beta.9'.
    """
    import re
    v = (v or "").strip()
