...
<dict>
    ...
    <!-- arm64 二进制与 Python 3.11 都需要 macOS 11 以上。原来写的 10.14 是错的,
         会让 10.14/10.15 的用户以为能跑。Intel 包同样按 11.0 声明 —— 实际构建
         用的 SDK 也决定了更低版本跑不了。
         arm64 binaries and Python 3.11 both require macOS 11+. The previous value of
         10.14 was simply wrong and would lead 10.14/10.15 users to believe it would
         run. The Intel build declares the same, since the SDK it is built against
         rules out anything older. -->
...
#
# 后果不是警告,是下载者看到「PrintTheShot.app 已损坏,无法打开」—— 而且右键
# 打开也救不了,因为那不是"未验证开发者",是签名本身校验失败。
#
...
#     -> code has no resources but signature indicates they must be present
#
# The result is not a warning: every downloader sees "PrintTheShot.app is damaged
# and can't be opened", and right-clicking does not help, because this is not an
# "unidentified developer" block — the signature itself fails to validate.
...
# permission bits, which is what .app packaging on macOS requires. zip drops them,
# and the unpacked bundle can end up missing its executable bit.
